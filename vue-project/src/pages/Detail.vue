<template>
  ...
  <script>
    import ImageList from '@/components/Detail/ImageList/ImageList.vue'
    import Zoom from '@/components/Detail/Zoom/Zoom.vue'
    ...
    export default {
      ...
      methods:{
        ...
        async addToChart(){
          ...
          // 上面代码实质是调用仓库中的addOrUpdateShopCart函数
          ...
          try {
            ...
            {skuid:this.$route.params.skuid,skuNum:this.skuNum})
            // 路由跳转
            // 路由跳转时，需要将产品信息带给下一级的路由组件
            // 下面这种方式可以在路由跳转时传递参数

            // 产品信息数据比较复杂，通过会话存储（不持久化，会话结束就消失）
            // 本地存储|存储，一般都是字符串
            sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo))
            // 简单数据skuNum,通过query形式传递过去
            this.$router.push(
              {name:'addcartsuccess',
